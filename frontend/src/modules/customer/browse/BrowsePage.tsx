        ...
        <div className="fade-in" style={{ fontFamily: 'Poppins, sans-serif', backgroundColor: '#FAFAFA', minHeight: '100vh' }}>
            ...
            <main style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 24px' }}>
                <div style={{ marginBottom: 32 }}>
                    <h1 style={{ fontSize: 28, fontWeight: 700, color: '#111827' }}>Browse Restaurants</h1>
                    <p style={{ fontSize: 15, color: '#4B5563', marginTop: 4 }}>Discover exceptional dining experiences near you.</p>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 24, alignItems: 'center' }}>
                    <div style={{ position: 'relative', flex: 1 }}>
                        ...
                        <input
                            ...
                            style={{ width: '100%', padding: '14px 16px 14px 48px', border: '1px solid #E5E7EB', borderRadius: 12, fontSize: 14, fontFamily: 'Poppins', outline: 'none', background: 'white' }}
                        />
                    </div>
                    <button onClick={() => setShowFilters(!showFilters)}
                        style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 20px', border: '1px solid #E5E7EB', borderRadius: 12, background: showFilters ? '#FEF2F2' : 'white', color: showFilters ? '#B91C1C' : '#374151', fontSize: 14, cursor: 'pointer', fontFamily: 'Poppins', fontWeight: 500 }}>
                    ...
                    <div style={{ display: 'flex', border: '1px solid #E5E7EB', borderRadius: 12, overflow: 'hidden', backgroundColor: 'white', padding: 3, gap: 2 }}>
                        {(['grid', 'list'] as const).map(v => (
                            <button key={v} onClick={() => setViewMode(v)}
                                style={{ padding: '10px 14px', border: 'none', borderRadius: 8, background: viewMode === v ? '#B91C1C' : 'transparent', color: viewMode === v ? 'white' : '#6B7280', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div className="no-scrollbar" style={{ display: 'flex', gap: 8, marginBottom: 24, overflowX: 'auto', paddingBottom: 6 }}>
                    {CUISINES.map(c => (
                        <button key={c} onClick={() => { setCuisine(c); setPage(1); }}
                            style={{ padding: '8px 20px', borderRadius: 100, border: '1px solid', borderColor: cuisine === c ? '#B91C1C' : '#E5E7EB', background: cuisine === c ? '#B91C1C' : 'white', color: cuisine === c ? 'white' : '#4B5563', fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins', whiteSpace: 'nowrap', fontWeight: cuisine === c ? 600 : 500 }}>
                            {c}
                        </button>
                    ))}
                </div>
                ...
                {showFilters && (
                    <div style={{ background: 'white', borderRadius: 16, border: '1px solid #E5E7EB', padding: 24, marginBottom: 28 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                            <div style={{ fontWeight: 600, fontSize: 14, color: '#111827', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Filters</div>
                            {hasFilters && (
                                <button onClick={clearFilters} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', color: '#B91C1C', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                                    <X size={14} /> Clear all
                                </button>
                            )}
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                            <div>
                                <div style={{ fontWeight: 500, fontSize: 13, color: '#374151', marginBottom: 10 }}>Price Range</div>
                                <div style={{ display: 'flex', gap: 8 }}>
                                    {PRICE_RANGES.map(p => (
                                        <button key={p} onClick={() => { setPriceRange(priceRange === p ? '' : p); setPage(1); }}
                                            style={{ width: 56, height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid', borderColor: priceRange === p ? '#B91C1C' : '#E5E7EB', borderRadius: 10, background: priceRange === p ? '#FEF2F2' : 'white', color: priceRange === p ? '#B91C1C' : '#374151', fontSize: 14, cursor: 'pointer', fontFamily: 'Poppins', fontWeight: 600 }}>
                                            {p}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div>
                                <div style={{ fontWeight: 500, fontSize: 13, color: '#374151', marginBottom: 10 }}>Sort By</div>
                                <select value={sort} onChange={e => { setSort(e.target.value); setPage(1); }}
                                    style={{ width: '100%', height: 42, padding: '0 12px', border: '1px solid #E5E7EB', borderRadius: 10, fontSize: 13, fontFamily: 'Poppins', outline: 'none', background: 'white', cursor: 'pointer' }}>
                ...
                {hasFilters && (
                    <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        <button onClick={clearFilters} style={{ fontSize: 12, color: '#B91C1C', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Clear all</button>
                    </div>
                )}

                <div style={{ fontSize: 14, color: '#6B7280', marginBottom: 20, fontWeight: 500 }}>
                    Showing <span style={{ color: '#111827', fontWeight: 600 }}>{restaurants.length}</span> of <span style={{ color: '#111827', fontWeight: 600 }}>{data?.total || restaurants.length}</span> restaurants
                </div>
...
function FilterTag({ label, onRemove }: { label: string; onRemove: () => void }) {
    return (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 9999, background: '#FEE2E2', color: '#B91C1C', fontSize: 12, fontWeight: 500 }}>
            {label}
            <button onClick={onRemove} style={{ background: 'none', border: 'none', color: '#B91C1C', cursor: 'pointer', padding: 0, display: 'flex' }}><X size={12} /></button>
        </span>
    );
}
...
function EmptyState({ hasFilters, onClearFilters }: { hasFilters: boolean; onClearFilters: () => void }) {
    return (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: 'white', borderRadius: 16, border: '1px solid #E5E7EB' }}>
            ...
            <p style={{ fontSize: 14, color: '#6B7280', maxWidth: 400, margin: '0 auto 20px', lineHeight: 1.6 }}>
            ...
            {hasFilters && (
                <button onClick={onClearFilters} style={{ padding: '10px 24px', background: '#B91C1C', color: 'white', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 14, cursor: 'pointer', fontFamily: 'Poppins' }}>
                    Clear Filters
                </button>
            )}
        </div>
    ...
    return (
        <div onClick={onClick}
            ...
            <div style={{ position: 'relative', overflow: 'hidden' }}>
                ...
                <span style={{ position: 'absolute', top: 14, left: 14, backgroundColor: r.status === 'active' ? '#10B981' : '#EF4444', color: 'white', fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 6 }}>
                    {r.status === 'active' ? 'Open Now' : 'Closed'}
                </span>
            </div>
            <div style={{ padding: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                    <div style={{ fontWeight: 700, fontSize: 17, color: '#111827' }}>{r.name}</div>
                ...
                <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {(r.city || r.country) && (<><span style={{ color: '#D1D5DB' }}>·</span><span>{r.city}{r.city && r.country ? ', ' : ''}{r.country}</span></>)}
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <button onClick={(e) => { e.stopPropagation(); onMenu(); }} style={{ flex: 1, padding: '11px', border: '1px solid #D1D5DB', borderRadius: 10, background: 'white', color: '#374151', fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins', fontWeight: 500 }}>View Menu</button>
                    <button onClick={(e) => { e.stopPropagation(); onBook(); }} style={{ flex: 1, padding: '11px', border: 'none', borderRadius: 10, background: '#B91C1C', color: 'white', fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins', fontWeight: 600 }}>Book Table</button>
                </div>
            </div>
        </div>
    );
...
function RestaurantListItem({ restaurant: r, onClick, onMenu, onBook }: { restaurant: BrowseRestaurant; onClick: () => void; onMenu: () => void; onBook: () => void }) {
    return (
        <div onClick={onClick}
            style={{ background: 'white', borderRadius: 16, border: '1px solid #E5E7EB', padding: 20, display: 'flex', gap: 20, cursor: 'pointer', alignItems: 'center' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 140 }}>
                <button onClick={(e) => { e.stopPropagation(); onMenu(); }} style={{ padding: '10px 16px', border: '1px solid #D1D5DB', borderRadius: 10, background: 'white', color: '#374151', fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins', fontWeight: 500 }}>Menu</button>
                <button onClick={(e) => { e.stopPropagation(); onBook(); }} style={{ padding: '10px 16px', border: 'none', borderRadius: 10, background: '#B91C1C', color: 'white', fontSize: 13, cursor: 'pointer', fontFamily: 'Poppins', fontWeight: 600 }}>Book Table</button>
            </div>
        </div>
    );
}
