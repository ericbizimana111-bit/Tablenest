        ...
        <div className="fade-in">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
                <div>
                    ...
                    <p style={{ fontSize: 13, color: '#475569' }}>Drive more bookings with strategic culinary incentives.</p>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
                {(promos.length ? promos : []).map((p: Promotion) => (
                    <div key={p._id} style={{ background: 'white', borderRadius: 12, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
                        <div style={{ background: p.isActive ? '#F97316' : '#94A3B8', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ padding: 16 }}>
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                                {(p.applicableCategories || []).map((cat: string) => (
                                    <span key={cat} style={{ background: '#F1F5F9', color: '#475569', padding: '3px 10px', borderRadius: 9999, fontSize: 12 }}>{cat}</span>
                                ))}
                            </div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <div style={{ display: 'flex', gap: 6 }}>
                                    <button onClick={() => openEdit(p)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569', padding: 6 }}><Pencil size={14} /></button>
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editPromo ? 'Edit Promotion' : 'New Promotion'}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <div>
                        ...
                        <div style={{ border: '1.5px solid #E2E8F0', borderRadius: 8, padding: 10 }}>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: form.applicableCategories.length ? 8 : 0 }}>
                                {form.applicableCategories.map((cat, i) => (
                                    <span key={i} style={{ background: '#F1F5F9', color: '#475569', padding: '3px 10px', borderRadius: 9999, fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
                                        {cat} <button onClick={() => removeCat(i)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8', padding: 0, fontSize: 14, lineHeight: 1 }}>×</button>
                                    </span>
                                ))}
                                <button onClick={addCat} style={{ background: 'none', border: '1.5px dashed #F97316', borderRadius: 9999, padding: '3px 10px', fontSize: 12, color: '#F97316', cursor: 'pointer', fontFamily: 'Poppins' }}>+ Add Category</button>
                            </div>
                            {form.applicableCategories.length === 0 && (
                                <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                                    <input value={catInput} onChange={e => setCatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addCat()} placeholder="Category name..."
                                        style={{ flex: 1, padding: '6px 10px', border: '1px solid #E2E8F0', borderRadius: 6, fontSize: 13, fontFamily: 'Poppins', outline: 'none' }} />
                                    <button onClick={addCat} style={{ padding: '6px 12px', background: '#F97316', color: 'white', border: 'none', borderRadius: 6, fontSize: 12, cursor: 'pointer', fontFamily: 'Poppins' }}>Add</button>
                                </div>
                            )}
                            <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 6 }}>Applied to all items in selected categories</div>
                        </div>
                    </div>
