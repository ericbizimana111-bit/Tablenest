        ...
        <div style={{ fontFamily: 'Poppins, sans-serif', background: '#F8FAFC', overflowX: 'hidden' }}>
            ...
            <section className="animate-rain-fade" style={{ marginTop: 75, background: 'linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.6)), url(https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=1600&q=80) center/cover no-repeat', minHeight: 560, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', padding: '60px 20px', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 0, background: 'white', borderRadius: 12, overflow: 'hidden', boxShadow: '0 12px 40px rgba(0,0,0,0.25)', maxWidth: 720, width: '100%', padding: 6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', padding: '0 16px', borderRight: '1px solid #E2E8F0', flex: 0.8 }}>
                    ...
                    <button onClick={() => navigate(`/restaurants?search=${cuisine}&location=${location}`)} className="btn-hover"
                        style={{ background: '#F97316', color: 'white', border: 'none', padding: '0 32px', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: 14, fontFamily: 'Poppins' }}>
                        Find a Table
                    </button>
                </div>
            </section>
            ...
            <section className="animate-rain-fade" style={{ padding: '80px 80px', background: 'white', animationDelay: '0.1s' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 32 }}>
                    {[
                        ...
                        <div key={f.title} className="hover-card" style={{ border: '1px solid #E2E8F0', borderRadius: 16, padding: 32, background: '#FFF' }}>
                            <div style={{ background: '#FEE2E2', color: '#F97316', width: 52, height: 52, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>{f.icon}</div>
                            <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 10, color: '#0F172A' }}>{f.title}</div>
            ...
            {/* How It Works */}
            <section className="animate-rain-fade" style={{ padding: '80px 80px', background: '#FFF7ED', animationDelay: '0.2s' }}>
                <h2 style={{ textAlign: 'center', fontSize: 32, fontWeight: 800, marginBottom: 60, color: '#0F172A', letterSpacing: '-0.5px' }}>How It Works</h2>
                <div style={{ display: 'flex', gap: 0, alignItems: 'flex-start', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: 28, left: '16%', right: '16%', height: 2, background: '#E2E8F0', zIndex: 0 }} />
                    {STEPS.map(s => (
                        <div key={s.n} style={{ flex: 1, textAlign: 'center', position: 'relative', zIndex: 1 }}>
                            <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#F97316', color: 'white', fontWeight: 700, fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', boxShadow: '0 4px 10px rgba(249, 115, 22, 0.25)' }}>{s.n}</div>
                            <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 8, color: '#0F172A' }}>{s.title}</div>
                            <p style={{ fontSize: 14, color: '#475569', maxWidth: 220, margin: '0 auto', lineHeight: 1.5 }}>{s.desc}</p>
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="animate-rain-fade" style={{ padding: '80px 80px', background: 'white', animationDelay: '0.3s' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 36 }}>
                    <h2 style={{ fontSize: 32, fontWeight: 800, color: '#0F172A', letterSpacing: '-0.5px' }}>Popular Near You</h2>
                    <span onClick={() => navigate('/restaurants')} className="nav-link" style={{ color: '#F97316', fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>View All <ArrowRight size={16} /></span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 24 }}>
                    {(featuredRestaurants || []).map((r: any) => (
                        <div key={r.name} className="hover-card" style={{ border: '1px solid #E2E8F0', borderRadius: 16, overflow: 'hidden', cursor: 'pointer', background: 'white', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }} onClick={() => navigate(`/restaurants/${r._id}`)}>
                            <div style={{ position: 'relative' }}>
                                ...
                                <span style={{ position: 'absolute', top: 12, left: 12, background: r.status === 'active' ? '#16A34A' : '#DC2626', color: 'white', fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 9999, backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', gap: 4 }}>
                                    <span style={{ fontSize: 8 }}>●</span> {r.status === 'active' ? 'Open' : 'Closed'}
                                </span>
                            </div>
                            <div style={{ padding: 18 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                    <span style={{ fontWeight: 700, fontSize: 16, color: '#0F172A' }}>{r.name}</span>
                                    <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 14, color: '#475569', fontWeight: 500 }}><Star size={14} fill="#F59E0B" color="#F59E0B" />{r.rating}</span>
                                </div>
                                <div style={{ fontSize: 13, color: '#475569', marginBottom: 16 }}>{r.cuisineType || "Various"} · {r.priceRange || '$$'}{r.city ? ` · ${r.city}` : ''}</div>
                                <div style={{ display: 'flex', gap: 10 }}>
                                    <button className="btn-outline-hover" style={{ flex: 1, padding: "9px", border: "1px solid #E2E8F0", borderRadius: 8, background: "white", fontSize: 13, cursor: "pointer", fontFamily: "Poppins", fontWeight: 600, color: "#475569" }} onClick={(e) => { e.stopPropagation(); navigate("/restaurants/" + r._id); }}>View Menu</button>
                                    <button className="btn-hover" style={{ flex: 1, padding: "9px", border: "none", borderRadius: 8, background: "#F97316", color: "white", fontSize: 13, cursor: "pointer", fontFamily: "Poppins", fontWeight: 600 }} onClick={(e) => { e.stopPropagation(); navigate("/restaurants/" + r._id); }}>Book</button>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <section className="animate-rain-fade" style={{ padding: '60px 0', background: '#FFF7ED', animationDelay: '0.4s', overflow: 'hidden' }}>
                <h2 style={{ textAlign: 'center', fontSize: 32, fontWeight: 800, marginBottom: 44, color: '#0F172A', letterSpacing: '-0.5px' }}>Explore by Cuisine</h2>
                    ...
                    <div className="cuisine-track">
                        {[...CUISINE_DATA, ...CUISINE_DATA].map((c, i) => (
                            <div key={`${c.name}-${i}`} onClick={() => navigate(`/restaurants?cuisine=${c.name}`)} className="cuisine-item" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, cursor: 'pointer', width: 100 }}>
                                ...
                                <span style={{ fontSize: 13, fontWeight: 600, color: '#475569', textAlign: 'center', whiteSpace: 'nowrap' }}>{c.name}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section className="animate-rain-fade" style={{ padding: '80px 80px', background: '#F8FAFC', animationDelay: '0.5s' }}>
                <h2 style={{ textAlign: 'center', fontSize: 32, fontWeight: 800, marginBottom: 48, color: '#0F172A', letterSpacing: '-0.5px' }}>What Diners Are Saying</h2>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 28 }}>
                    {TESTIMONIALS.map(t => (
                        <div key={t.name} className="hover-card" style={{ border: '1px solid #E2E8F0', borderRadius: 16, padding: '32px 32px 28px', background: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', position: 'relative', overflow: 'hidden' }}>
                            <div style={{ position: 'absolute', top: 0, left: 0, width: 4, height: '100%', background: '#F97316', borderRadius: '16px 0 0 16px' }} />
                            <p style={{ fontSize: 15, color: '#475569', lineHeight: 1.7, fontStyle: 'italic', fontWeight: 400, margin: '0 0 24px 8px' }}>"{t.text}"</p>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: 12, color: '#F97316', fontWeight: 500 }}>{t.role}</div>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {/* CTA Banner Section */}
            <section className="animate-rain-fade" style={{ margin: '0 80px 80px', background: 'linear-gradient(135deg, #EA580C 0%, #EA580C 100%)', borderRadius: 20, padding: '50px 60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 10px 30px rgba(194, 65, 12, 0.25)', animationDelay: '0.6s' }}>
                <div>
